...
function update(data, type, reverse){
  ...
  d3.select(".chart")
    ...
    .attr("tabindex", 0)
    .attr("aria-label", type === "stores" ? "Bar chart showing coffee house chains ranked by number of stores worldwide" : "Bar chart showing coffee house chains ranked by revenue in billions of US dollars");
  svg.selectAll(".bar")
     .attr("role", "graphics-symbol")
     .attr("aria-roledescription", "bar element")
     .attr("tabindex", 0)
     .attr("aria-label", d => {return type === "stores" ? `${d.company}'s number of stores worldwide is ${d.stores}` : `${d.company}'s revenue is ${d.revenue} billion US dollars`;});
}
